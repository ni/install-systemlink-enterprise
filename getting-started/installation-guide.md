...
A cluster administrator with full access rights must use the systemlink-admin Helm chart to install prerequisite resources globally on the cluster. \<systemlink-admin\> namespace. Download a copy of [systemlink-admin-values.yaml](templates/admin-values.yaml) to deploy the configuration for these prerequisites. You will not need to modify any of the defaults in this file.
